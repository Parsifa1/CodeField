...
const constexpr ll bit_pow[20] = {
    1,        (ll)10,   (ll)1e2,  (ll)1e3,  (ll)1e4,  (ll)1e5,  (ll)1e6,
    (ll)1e7,  (ll)1e8,  (ll)1e9,  (ll)1e10, (ll)1e11, (ll)1e12, (ll)1e13,
    (ll)1e14, (ll)1e15, (ll)1e16, (ll)1e17, (ll)1e18};
template <typename TP>
inline void read(TP &n) {
    TP w = 1;
    char x = getchar();
    n = 0;
    while (x < '0' || x > '9') {
...
inline void read(TP &n, Etc &...etcs) {
    read(n);
    read(etcs...);
}
/*BIGNUM*/
...
struct bignum {
    ...
    void input(char k = ' ') {
        ...
        for (int i = s.size() - 1; i >= 1; i--) {
            s[i] += (s[i - 1] % bit_pow[bit - rest] * bit_pow[rest]);
            s[i - 1] /= bit_pow[bit - rest];
        }
        reverse();
        del();
    }
    void output() {
        if (negative) putchar('-');
        write(s[s.size() - 1]);
        for (int i = s.size() - 2, put; i >= 0; i--) {
            put = bit - get(s[i]);
            while (put-- > 0) putchar('0');
            write(s[i]);
        }
    }
    void reverse() {
    ...
    void del() {
        while (s.size() > 1 && !(*(--s.end()))) s.pop_back();
    ...
    ll number() {
        ll K = 0;
        for (int i = s.size() - 1; i >= 0; i--) K = K * bit_pow[bit] + s[i];
        return negative ? -K : K;
    }
    short operator[](int x) {
        return s[x / bit] % bit_pow[x % bit + 1] / bit_pow[x % bit];
    }
    ...
    bignum operator=(const bool x) {
        clear();
        s.emplace_back(x);
        return *this;
    }
    bignum operator=(const char x) {
        clear();
        s.emplace_back(x - '0');
        return *this;
    }
    ...
    bignum operator=(unsigned short x) {
        clear();
        do {
            s.emplace_back(x % bit_pow[bit]);
        } while (x /= bit_pow[bit]);
    ...
    bignum operator=(const char *x) {
        ...
        int len = strlen(x);
        int rest = len % bit;
        s.resize((len / bit) + (len % bit != 0));
        rest += (!rest) * bit;
        int CJB = s.size() - 1;
        for (int i = 0; i < len; i++) {
            s[CJB] = (s[CJB] << 1) + (s[CJB] << 3) + (x[i] - '0');
            if (!(--rest)) CJB--, rest = bit;
        }
        del();
        return *this;
    }
...
bool operator<(const bignum &x, const bignum &y) {
    ...
    if (x.s.size() != y.s.size()) return (x.s.size() < y.s.size()) ^ x.negative;
    for (int i = x.s.size() - 1; i >= 0; i--)
        if (x.s[i] != y.s[i]) return (x.s[i] < y.s[i]) ^ x.negative;
    return false;
}
...
bool operator==(const bignum &x, const bignum &y) {
    if (x.negative != y.negative || x.s.size() != y.s.size()) return false;
    for (int i = x.s.size() - 1; i >= 0; i--)
...
bignum operator+(const bignum &x, const bignum &y) {
    ...
    if (x.negative == y.negative)
        ...
        ANS = minus(max(abs(x), abs(y)), min(abs(x), abs(y))),
        ANS.negative =
            (x.negative && abs(x) > abs(y)) || (!x.negative && abs(y) > abs(x));
    return ANS;
}
bignum operator-(const bignum &x, const bignum &y) {
    bignum ANS;
...
bignum operator*(const bignum &x, const bignum &y) {
    ...
    for (unsigned int i = 0; i < ANS.s.size(); i++) ANS.s[i] = 0;
...
bignum operator/(bignum x, const bignum &y) {
    ...
    while (r >= 0) {
        while (bignum_Compare(l, r, x.s, y.s)) {
            for (int i = r;
                 i - r < y.s.size() && i <= l - (l - r == y.s.size()); i++) {
                x.s[i] -= y.s[i - r];
                x.s[i + 1] -= (x.s[i] < 0);
...
bignum operator/(bignum x, const bignum &y) {
    ...
    while (r >= 0) {
        ...
        while (L <= R) {
            Type MID = (L + R) >> 1;
            if (bignum_Compare(l, r, x.s, (y * MID).s))
                ans = MID, L = MID + 1;
            else
                R = MID - 1;
        }
...
bignum jiecheng(int x) {
    bignum ret = 1;
    for (int i = x; i > 0; i--) ret *= i;
    return ret;
}

bignum C(int x, int y) {
    bignum ret = 0;
    bignum x1 = jiecheng(x);
    bignum x2 = jiecheng(x - y) * 2;
    ret = x1 / x2;
    return ret;
}
